...
import "./Excellence.css"; // your CSS
  ...
  useEffect(() => {
    ...
    counterRefs.current.forEach(counter => {
      ...
      const updateCounter = () => {
        ...
        counter.innerHTML = `${current}${hasPlus ? "+" : ""} <span>${spanText}</span>`;
  ...
  useEffect(() => {
    ...
    svg.querySelectorAll("line").forEach(line => {
      ...
      [[x1, y1], [x2, y2]].forEach(([x, y]) => {
        if (![...svg.querySelectorAll("circle")].some(c =>
          Math.abs(c.getAttribute("cx") - x) < 1 && Math.abs(c.getAttribute("cy") - y) < 1
        )) {
        ...
        <div className="image-content">
          <div className="since-box">
            <svg
              ...
              viewBox="0 0 400 400"
            >
              {/* Add all your <line> elements here */}
